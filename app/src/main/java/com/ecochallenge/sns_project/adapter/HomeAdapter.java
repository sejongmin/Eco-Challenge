...
import com.ecochallenge.sns_project.FirebaseHelper;
import com.ecochallenge.sns_project.PostInfo;
import com.ecochallenge.sns_project.R;
import com.ecochallenge.sns_project.activity.PostActivity;
...
import com.ecochallenge.sns_project.view.ReadContentsVIew;
...
public class HomeAdapter extends RecyclerView.Adapter<HomeAdapter.MainViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final MainViewHolder holder, int position) {
        ...
        final TextView publisherTextView = cardView.findViewById(R.id.publisherNameTextView);//11/22 추가 구민
        final ImageView profileImageView = cardView.findViewById(R.id.profileImageView1);


        PostInfo postInfo = mDataset.get(position);
        titleTextView.setText(postInfo.getTitle());
        final String publisherName = postInfo.getId();

        FirebaseFirestore db = FirebaseFirestore.getInstance();
        DocumentReference docRef = db.collection("users").document(postInfo.getPublisher());
        docRef.get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    if (document.exists()) {
                        publisherTextView.setText(document.getData().get("name").toString());
                        if(document.getData().get("photoUrl") != null){
                            Glide.with(holder.itemView.getContext()).load(document.getData().get("photoUrl")).centerCrop().override(500).into(profileImageView);//.centerCrop().override(500)
                        }
